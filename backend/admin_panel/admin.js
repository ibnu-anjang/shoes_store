        ...
        function renderDashboard() {
            ...
            // Status breakdown
            const statusOrder = ['UNPAID', 'VERIFYING', 'PAID', 'SHIPPED', 'COMPLETED', 'DELIVERED', 'CANCELLED', 'REJECTED'];
            const statusColors = {
                UNPAID: 'bg-yellow-500', VERIFYING: 'bg-orange-500', PAID: 'bg-blue-500',
                SHIPPED: 'bg-purple-500', COMPLETED: 'bg-green-500', DELIVERED: 'bg-teal-500',
                CANCELLED: 'bg-red-500', REJECTED: 'bg-red-700'
            };
            const counts = {};
    ...
    `).join('');
        ...
        function addColorInput() {
            ...
            const existingValues = Array.from(list.querySelectorAll('.color-item input[type="color"]')).map(i => i.value.toLowerCase());
            if (existingValues.includes('#000000')) {
                toast('Warna #000000 sudah ada. Ubah warna yang ada atau pilih warna berbeda.', 'warn');
                return;
            }
            const div = document.createElement('div');
            div.className = 'color-item relative fade-in flex flex-col items-center gap-0.5';
            div.innerHTML = `
                <input type="color" value="#000000"
                    ...
                    oninput="this.nextElementSibling.textContent=this.value.toUpperCase()"
                    onchange="checkDuplicateColorAdd(this); this.nextElementSibling.textContent=this.value.toUpperCase(); updateVariationMatrix()">
                <span class="text-[9px] text-gray-400 font-mono select-all">#000000</span>
                <button type="button"
                    onclick="this.closest('.color-item').remove(); updateVariationMatrix();"
        ...
        function checkDuplicateColorAdd(inp) {
            const list = document.getElementById('p-color-list');
            const isDuplicate = Array.from(list.querySelectorAll('.color-item input[type="color"]'))
                .some(other => other !== inp && other.value.toLowerCase() === inp.value.toLowerCase());
            if (isDuplicate) {
                toast('Warna dengan kode ini sudah ada', 'warn');
                inp.value = '#000000';
                inp.nextElementSibling.textContent = '#000000';
            }
        }
        ...
        async function submitAddProduct(e) {
            ...
            const colorRawValues = Array.from(colorItems).map(inp => inp.value.toLowerCase());
            const uniqueColorRawValues = new Set(colorRawValues);
            if (uniqueColorRawValues.size !== colorRawValues.length) {
                toast('Terdapat warna duplikat! Hapus salah satunya sebelum menyimpan.', 'error');
                return;
            }
        ...
        function openPhotoModal(productId, productName) {
            ...
            if (p && p.gallery && p.gallery.length > 0) {
                ...
                p.gallery.forEach(img => {
                    ...
                    row.innerHTML = `
                        <div class="relative shrink-0 group/thumb cursor-pointer" onclick="openImgPreview('${normalizedUrl}')">
                            <img src="${normalizedUrl}" class="w-12 h-12 rounded object-cover border border-gray-600 group-hover/thumb:brightness-75 transition">
                            <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover/thumb:opacity-100 transition pointer-events-none">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span class="color-swatch-preview shrink-0 w-4 h-4 rounded border border-gray-500" style="display:none;background:#000"></span>
                                <select onchange="updateExistingPhotoRole(${img.id}, this.value); updateColorSwatch(this)" class="bg-gray-700 border border-gray-600 text-xs rounded px-2 py-1 text-white w-full outline-none focus:border-orange-500">
        ...
        function getColorFromValue(val) {
            if (!val || val === 'gallery' || val === 'thumbnail') return null;
            return val.startsWith('0xFF') ? '#' + val.slice(4) : val;
        }

        function updateColorSwatch(selectEl) {
            const swatch = selectEl.parentElement.querySelector('.color-swatch-preview');
            if (!swatch) return;
            const color = getColorFromValue(selectEl.value);
            if (color) {
        ...
        document.addEventListener('keydown', e => { if (e.key === 'Escape') closeImgPreview(); });
        function previewPhotoQueue(input) {
            ...
            files.forEach((file, idx) => {
                ...
                reader.onload = e => {
                    ...
                    row.innerHTML = `
                        <div class="relative shrink-0 group/thumb cursor-pointer" onclick="openImgPreview('${dataUrl}')">
                            <img src="${dataUrl}" class="w-12 h-12 rounded object-cover border border-gray-600 group-hover/thumb:brightness-75 transition">
                            <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover/thumb:opacity-100 transition pointer-events-none">
                                <svg class="w-5 h-5 text-white drop-shadow" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"/></svg>
                            </div>
                        </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span class="color-swatch-preview shrink-0 w-4 h-4 rounded border border-gray-500" style="display:none;background:#000"></span>
                                <select onchange="updateColorSwatch(this)" class="photo-role-select bg-gray-700 border border-gray-600 text-xs rounded px-2 py-1 text-white w-full outline-none focus:border-orange-500">
        ...
        function openEditModal(productId) {
            ...
            if (p.colors && p.colors.length) {
                p.colors.forEach(c => addColorInputEdit(c.color_hex, true));
            }
        ...
        function addColorInputEdit(hex = '#000000', isSaved = false) {
            const list = document.getElementById('edit-color-list');
            const hexForCompare = (hex.startsWith('0xFF') ? '#' + hex.slice(4) : hex).toLowerCase();
            
            // Only check for duplicates if we're adding a NEW color manually
            if (!isSaved) {
                const duplicateResource = Array.from(list.querySelectorAll('.color-item input[type="color"]'))
                    .some(inp => inp.value.toLowerCase() === hexForCompare);
                if (duplicateResource) { toast('Warna dengan kode ini sudah ada', 'warn'); return; }
            }
            const div = document.createElement('div');
            div.className = 'color-item relative fade-in flex flex-col items-center gap-0.5';
            div.dataset.saved = isSaved ? 'true' : 'false';
            ...
            div.innerHTML = `
                <input type="color" value="${hexVal}" data-prev="${hexApi}"
                    class="w-12 h-12 rounded-xl cursor-pointer border-2 border-gray-600 hover:border-orange-500 transition-all p-0.5 bg-transparent"
                    oninput="this.nextElementSibling.textContent=this.value.toUpperCase()"
                    onchange="this.nextElementSibling.textContent=this.value.toUpperCase(); handleColorChangeEdit(this)">
                <span class="text-[9px] text-gray-400 font-mono select-all">${hexVal.toUpperCase()}</span>
                <button type="button"
                    onclick="removeColorInputEdit(this)"
                    class="absolute -top-2 -right-2 w-5 h-5 bg-red-600 hover:bg-red-500 rounded-full text-white text-[10px] font-bold flex items-center justify-center shadow-lg border border-red-800 transition-colors">✕</button>
            `;
            list.appendChild(div);
            div.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        ...
        function handleColorChangeEdit(inp) {
            ...
            // Cek duplikat: ada color lain di list yang hex-nya sama
            const list = document.getElementById('edit-color-list');
            const isDuplicate = Array.from(list.querySelectorAll('.color-item input[type="color"]'))
                .some(other => other !== inp && other.value.toLowerCase() === newHexRaw.toLowerCase());
            if (isDuplicate) {
                toast('Warna dengan kode ini sudah ada', 'warn');
                const revertedVal = oldHex.startsWith('0xFF') ? '#' + oldHex.slice(4).toLowerCase() : oldHex;
                inp.value = revertedVal;
                inp.nextElementSibling.textContent = revertedVal.toUpperCase();
                return;
            }
            ...
            if (editingProductData && editingProductData.colors) {
                ...
                else if (!editingProductData.colors.some(c => c.color_hex === newHex)) {
                    editingProductData.colors.push({ color_hex: newHex });
                }
            }
        ...
        function removeColorInputEdit(btnElement) {
            const colorItem = btnElement.parentElement;
            const isSaved = colorItem.dataset.saved === 'true';
            const inp = colorItem.querySelector('input[type="color"]');
            const hexApi = inp ? inp.getAttribute('data-prev') : null;
            ...
            // Warna belum disimpan ke DB atau tidak punya SKU — hapus langsung tanpa sentuh DB
            colorItem.remove();
            updateSkuColorDropdownsEdit();
        }
        ...
        function updateSkuColorDropdownsEdit() {
            ...
            const hidden = document.getElementById('new-sku-color');
            const picker = document.getElementById('new-sku-color-picker');
            if (!hidden || !picker) return;

            const currentVal = hidden.value;
            if (!colors.includes(currentVal)) hidden.value = '';
            ...
            univBtn.type = 'button';
            univBtn.title = 'Universal / Semua';
            const isUniv = hidden.value === '';
            univBtn.className = `text-[10px] px-2 py-0.5 rounded font-semibold border transition-all ${isUniv ? 'bg-orange-500 border-orange-400 text-white' : 'bg-gray-700 border-gray-600 text-gray-300 hover:border-orange-500'}`;
            univBtn.textContent = 'Semua';
            univBtn.onclick = () => { hidden.value = ''; updateSkuColorDropdownsEdit(); };
            picker.appendChild(univBtn);

            colors.forEach(c => {
                const hexDisplay = c.replace('0xFF', '#');
                const btn = document.createElement('button');
                btn.type = 'button';
                btn.title = hexDisplay;
                const isSelected = hidden.value === c;
                btn.className = `relative w-6 h-6 rounded-full border-2 transition-all hover:scale-110 ${isSelected ? 'border-white scale-110 shadow-lg' : 'border-gray-600 hover:border-orange-400'}`;
                btn.style.backgroundColor = hexDisplay;
                btn.onclick = () => { hidden.value = c; updateSkuColorDropdownsEdit(); };
            ...
            if (colors.length === 0) {
                ...
                empty.textContent = 'Tambah warna dulu';
        ...
        function renderEditSkuList(skus) {
            ...
            groups.forEach((group) => {
                ...
                // Quick-add row per color group
                const groupColorHexApi = group.color || '';
                const defaultPrice = editingProductData.price || '';
                html += `
                <tr class="border-t border-gray-700/50">
                    <td class="py-1.5 pl-5">
                        <input type="text" placeholder="+ Ukuran (mis. 42)" data-qa-color="${groupColorHexApi}"
                            onkeydown="if(event.key==='Enter'){event.preventDefault();quickAddSku(this)}"
                            class="qa-size-input bg-gray-900 border border-dashed border-gray-600 rounded px-2 py-0.5 text-[11px] text-gray-300 placeholder-gray-600 w-24 focus:outline-none focus:border-orange-500">
                    </td>
                    <td class="py-1.5 pl-2">
                        <div class="bg-gray-900 border border-dashed border-gray-600 rounded-lg px-2 py-0.5 inline-flex items-center gap-1">
                            <span class="text-[9px] text-gray-500 font-bold">Rp</span>
                            <input type="text" placeholder="${defaultPrice}" data-qa-color="${groupColorHexApi}"
                                class="qa-price-input w-16 bg-transparent text-[11px] text-gray-300 placeholder-gray-600 focus:outline-none">
                        </div>
                    </td>
                    <td class="py-1.5 pl-2">
                        <div class="bg-gray-900 border border-dashed border-gray-600 rounded-lg px-2 py-0.5 inline-flex items-center gap-1">
                            <input type="number" placeholder="0" data-qa-color="${groupColorHexApi}" min="0"
                                class="qa-stock-input w-10 bg-transparent text-[11px] text-gray-300 placeholder-gray-600 text-center focus:outline-none">
                        </div>
                    </td>
                    <td class="py-1.5 pl-2 text-right">
                        <button onclick="quickAddSku(this.closest('tr').querySelector('.qa-size-input'))" class="p-1.5 text-orange-400 hover:bg-orange-500/10 rounded-lg transition-all" title="Tambah Ukuran">
        ...
        async function quickAddSku(sizeInput) {
            const row = sizeInput.closest('tr');
            const name = sizeInput.value.trim();
            if (!name) { sizeInput.focus(); return; }
            const color = sizeInput.dataset.qaColor || null;
            const priceRaw = row.querySelector('.qa-price-input').value;
            const stockRaw = row.querySelector('.qa-stock-input').value;
            const p = allProducts.find(x => x.id === currentEditProductId);
            const price = parsePrice(priceRaw) || (p ? p.price : 0);
            const stock = parseInt(stockRaw) || 0;

            try {
                const res = await fetch(`${API}/admin/products/${currentEditProductId}/skus`, {
                    method: 'POST', headers: getHeaders(),
                    body: JSON.stringify({ variant_name: name, color_hex: color || null, price, stock_available: stock, stock_reserved: 0 })
                });
                if (res.ok) {
                    toast(`Ukuran "${name}" ditambahkan!`);
                    await fetchProducts();
                    const updated = allProducts.find(x => x.id === currentEditProductId);
                    if (updated) { editingProductData = updated; renderEditSkuList(updated.skus); }
        ...
        async function submitEditProduct() {
            ...
            // Collect colors
            const colorInputs = Array.from(document.querySelectorAll('#edit-color-list .color-item input[type="color"]'));
            const colorValues = colorInputs.map(inp => inp.value.toLowerCase());
            const uniqueColorValues = new Set(colorValues);
            if (uniqueColorValues.size !== colorValues.length) {
                toast('Terdapat warna duplikat! Hapus salah satunya sebelum menyimpan.', 'error');
                return;
            }
            const colors = colorInputs.map(inp => ({ color_hex: '0xFF' + inp.value.replace('#', '').toUpperCase() }));
            ...
            document.querySelectorAll('#edit-sku-list tr.group').forEach(row => {
                ...
                const vPrice = parsePrice(row.querySelector(`input[id="edit-price-${id}"]`).value);
        ...
        function updatePromoSelectedCount() {
            ...
            if (actionsBar) {
                if (selected.length > 0) actionsBar.classList.remove('hidden');
